library angular2.src.core.compiler.template_parser;
...
import "package:angular2/src/core/compiler/selector.dart"
...
class TemplateParseVisitor implements HtmlAstVisitor {
  ...
  dynamic visitElement(HtmlElementAst element, Component component) {
    ...
    if (identical(preparsedElement.type, PreparsedElementType.NG_CONTENT)) {
      ...
      var elementExportAsVars = ListWrapper.filter(
          vars, (varAst) => identical(varAst.value.length, 0));
  ...
  bool _parseInlineTemplateBinding(
    ...
    if (attr.name == TEMPLATE_ATTR) {
      ...
      var key = StringWrapper.substring(attr.name, TEMPLATE_ATTR_PREFIX.length);
