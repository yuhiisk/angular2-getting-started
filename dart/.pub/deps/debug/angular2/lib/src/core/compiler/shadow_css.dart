library angular2.src.core.compiler.shadow_css;
...
class ShadowCss {
  ...
  */
  String shimStyle(String style, String selector, [String hostSelector = ""]) {
  ...
  */
  ...
  /** @internal */
  String _scopeRules(cssRules, String scopeSelector, String hostSelector) {
    var cssText = "";
      ...
      for (var i = 0; i < cssRules.length; i++) {
        ...
        if (DOM.isStyleRule(rule) || DOM.isPageRule(rule)) {
          cssText += this._scopeSelector(rule.selectorText, scopeSelector,
                  hostSelector, this.strictStyling) +
              " {\n";
          cssText += this._propertiesFromRule(rule) + "\n}\n\n";
        } else if (DOM.isMediaRule(rule)) {
          cssText += "@media " + rule.media.mediaText + " {\n";
          cssText +=
              this._scopeRules(rule.cssRules, scopeSelector, hostSelector);
          cssText += "\n}\n\n";
        } else {
  ...
  /** @internal */
  String _ieSafeCssTextFromKeyFrameRule(rule) {
  ...
  /** @internal */
  String _propertiesFromRule(rule) {
    ...
    var attrRe = new RegExp(r'[' + "'" + r'"]+|attr');
    if (rule.style.content.length > 0 &&
        !isPresent(RegExpWrapper.firstMatch(attrRe, rule.style.content))) {
      var contentRe = new RegExp(r'content:[^;]*;');
      cssText = StringWrapper.replaceAll(
          cssText, contentRe, "content: '" + rule.style.content + "';");
    }
...
_cssToRules(String cssText) {
  return DOM.cssToRules(cssText);
}

_withCssRules(String cssText, Function callback) {
